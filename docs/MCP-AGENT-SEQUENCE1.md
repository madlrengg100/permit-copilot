...
|---|---|---|---|
| **RDB** | 법령·규제조건·규칙 = **판정 기준값** | 건폐율·용적률 등 기준값 | Rule Engine의 **수치 판정**, Graph DB 적재 원천 |
| **Graph DB** | RDB에서 적재한 **법령 간 연계정보 = 룰셋** | 산정방식 룰셋 · 그래프 확장 | Rule Engine의 **판정 규칙**, RAG의 **근거 확장** |
...
- 즉 `RDB→Graph→Vector` 직렬이 아니라, **RDB는 적재 원천**이고 **조회는 Graph·Vector 양쪽**에서
