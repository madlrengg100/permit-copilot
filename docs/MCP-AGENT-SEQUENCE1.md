...
| 회차 | Sub가 지시한 항목 | 에이전트가 채워 반환한 값 | State 잔여 |
|---|---|---|---|
| 1 | 대상 필지 확정 | PNU 4113510300102340001 · 대지면적 1,250㎡ · 경계 좌표 | 용도지역 · 기준값 · 규제구역 |
| 2 | 용도지역 확인 | 계획관리지역 (중첩 없음) | 기준값 · 규제구역 |
| 3 | 기준값 조회 | 건폐율 40% · 용적률 100% · 공장 허용(조건부) | 규제구역 |
| 4 | 규제구역 저촉 확인 | 저촉 없음 → State 완결 | 없음 (루프 종료) |

State가 다 채워지면 Sub는 더 실행하지 않고 그 기록을 통합 오케스트레이터에 올린다. 3회차에서
...
값으로 정해진 식을 계산한다(산출). 위 2·3회차에서 사전진단 추론 에이전트가 실행하는 모듈은
다음과 같다.

| 모듈 기능 | 구분 | 입력 | 출력 |
|---|---|---|---|
| 주소 정규화 | 전처리 | "○○시 △△구 □□로 123" | 도로명 코드 + 건물번호 |
| 필지 식별 | 전처리 | 정규화된 주소 | PNU 4113510300102340001 |
| 용도지역 응답 파싱 | 후처리 | 토지특성 조회 도구의 응답 | 용도지역 = 계획관리지역 |
| 유효 대지면적 산출 | 산출 | 대지면적 1,250㎡, 도로 점용부 50㎡ | 유효 대지면적 1,200㎡ |
| 규모 산정 | 산출 | 유효 대지면적 1,200㎡ · 건폐율 40% · 용적률 100% | 건축면적 상한 480㎡ · 연면적 상한 1,200㎡ |
| 판정 대조 | 후처리 | 공장 허용 여부 · 산출 규모 | 가능(조건부) · 조건 = 진입도로 폭 확보 |
...
| MCP Tool | 제공 시스템 | 요청 입력 | 반환 결과(Observation) |
|---|---|---|---|
| 지오코딩 | GIS | 도로명 코드 + 건물번호 | PNU · 중심좌표 · 필지 경계 |
| 토지특성 조회 | GIS | PNU | 지목 · 대지면적 1,250㎡ · 용도지역 계획관리지역 |
| 규제구역 저촉 조회 | GIS | PNU · 필지 경계 | 저촉 구역 목록 (해당 없음이면 빈 값) |
| 법령 기준값 조회 | 법령 DB | 용도지역 코드 + 시설물 용도(공장) | 건폐율 40% · 용적률 100% · 허용 여부(조건부) |
| RAG 근거 검색 | Vector·Graph DB | 판정 항목 + 용도지역 | 근거 조문 인용 · 유사사례 |
| 3D 매스 생성 | 3D 엔진 | 건축면적 480㎡ · 연면적 1,200㎡ | 매스 지오메트리 · 토공량 |
