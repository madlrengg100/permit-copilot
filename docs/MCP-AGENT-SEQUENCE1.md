...
모듈이 만든 입력은 MCP Tool 호출로 나간다. 도구는 GIS·법령 DB·3D 엔진 같은 기존 시스템의
기능이며, 호출 결과는 정해진 형식(Observation)으로 돌아온다. 위 예시 질의 한 건에서 실제로
호출되는 도구와 그 입출력은 다음과 같다.
...
| 4 | 법령 기준값 조회 | 법령 DB | 판정 대조 (사전진단) | 용도지역 코드 + 시설물 용도(공장) | 건폐율 40% · 용적률 100% · 허용 여부(조건부) |
| 5 | RAG 근거 검색 | Vector·Graph DB | 판정 대조 (사전진단) | 판정 항목 + 용도지역 | 근거 조문 인용 · 유사사례 |
| 6 | 3D 매스 생성 (선택) | 3D 엔진 | 매스 생성 (3D 시뮬레이션) | 건축면적 480㎡ · 연면적 1,200㎡ | 매스 지오메트리 · 토공량 |
...
sequenceDiagram
    ...
    MD->>LAW: ④ 법령 기준값 조회 (용도지역 코드 + 공장)
    LAW-->>MD: 건폐율 40% · 용적률 100% · 허용 여부(조건부)
...
정리하면 질의 한 건은 **Sub의 네 차례 실행 지시 → 여섯 개 모듈 기능 → 여섯 개 MCP Tool 호출**을
