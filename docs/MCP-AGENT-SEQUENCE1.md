...
△△구 □□로 123에 공장을 지을 수 있어?"* 이며, 담당은 사전진단 도메인이다. 아래 수치는 설명을 위한
예시값이다.

#### 5.2.1 Sub ↔ 에이전트 — 실행 지시와 State 반환

Sub는 통합 오케스트레이터에게 받은 요청을 실행 지시로 바꿔 에이전트에 넘긴다. 지시에는 대상
필지(PNU), 시설물 용도(공장), 이번에 채워야 할 항목 목록이 담긴다. 에이전트는 항목을 하나씩 채워
State에 반영하고, Sub는 비어 있는 항목이 남아 있으면 같은 에이전트를 다시 실행한다. 사전진단
도메인에서 이 반복은 아래와 같이 네 번 일어난다.

| 단계 | Sub가 지시한 항목 | 담당 | 에이전트가 채워 반환한 값 | State 잔여 |
|---|---|---|---|---|
| 1 | 대상 필지 확정 | 사전진단 추론 에이전트 | PNU 4113510300102340001 · 대지면적 1,250㎡ · 경계 좌표 | 용도지역 · 기준값 · 규제구역 |
| 2 | 용도지역 확인 | 사전진단 추론 에이전트 | 계획관리지역 (중첩 없음) | 기준값 · 규제구역 |
| 3 | 기준값 조회 | 사전진단 추론 에이전트 | 건폐율 40% · 용적률 100% · 공장 허용(조건부) | 규제구역 |
| 4 | 규제구역 저촉 확인 | 사전진단 추론 에이전트 | 저촉 없음 → State 완결 | 없음 (루프 종료) |
...
sequenceDiagram
    ...
    IO->>SO: 요청 (대상 필지 · 시설물 용도 = 공장)
    Note over SO: 요청을 실행 지시로 변환<br/>(대상 필지 · 용도 · 이번에 채울 항목)

    Note over SO,AG: State의 빈 항목이 없어질 때까지 같은 에이전트를 다시 실행한다

    SO->>AG: 단계 1 — 대상 필지 확정
    AG-->>SO: PNU 4113510300102340001 · 대지면적 1,250㎡ · 경계 좌표
    Note right of SO: 잔여 = 용도지역 · 기준값 · 규제구역
...
#### 5.2.2 에이전트 ↔ 모듈 — 기능 단위 분해와 값 추출

에이전트는 지시 하나를 곧바로 도구 호출로 바꾸지 않고, 기능 단위인 모듈로 나눠 실행한다. 모듈은
도구에 넣을 입력을 만들거나(전처리), 도구가 돌려준 응답에서 필요한 값만 뽑거나(후처리), 뽑은
값으로 정해진 식을 계산한다(산출). 위 2·3단계에서 사전진단 추론 에이전트가 실행하는 모듈은
다음과 같다.

| 순서 | 모듈 기능 | 구분 | 입력 | 출력 |
|---|---|---|---|---|
| 1 | 주소 정규화 | 전처리 | "○○시 △△구 □□로 123" | 도로명 코드 + 건물번호 |
| 2 | 필지 식별 | 전처리 | 정규화된 주소 | PNU 4113510300102340001 |
| 3 | 용도지역 응답 파싱 | 후처리 | 토지특성 조회 도구의 응답 | 용도지역 = 계획관리지역 |
| 4 | 유효 대지면적 산출 | 산출 | 대지면적 1,250㎡, 도로 점용부 50㎡ | 유효 대지면적 1,200㎡ |
| 5 | 규모 산정 | 산출 | 유효 대지면적 1,200㎡ · 건폐율 40% · 용적률 100% | 건축면적 상한 480㎡ · 연면적 상한 1,200㎡ |
| 6 | 판정 대조 | 후처리 | 공장 허용 여부 · 산출 규모 | 가능(조건부) · 조건 = 진입도로 폭 확보 |
...
sequenceDiagram
    ...
    participant SO as ② 사전진단 Sub
    participant AG as ③ 사전진단 추론 에이전트
    participant MP as ④ 모듈 (전처리)
    participant MQ as ④ 모듈 (후처리)
    participant MC as ④ 모듈 (산출)

    SO->>AG: 실행 지시 (2·3단계 — 용도지역 확인 · 기준값 조회)
    Note over AG: 지시를 곧바로 도구 호출로 바꾸지 않고<br/>기능 단위(모듈)로 나눠 실행 순서를 정한다

    AG->>MP: ① 주소 정규화 ("○○시 △△구 □□로 123")
    MP-->>AG: 도로명 코드 + 건물번호

    AG->>MP: ② 필지 식별 (정규화된 주소)
    MP-->>AG: PNU 4113510300102340001

    AG->>MQ: ③ 용도지역 응답 파싱 (토지특성 조회 도구의 응답)
    MQ-->>AG: 용도지역 = 계획관리지역

    AG->>MC: ④ 유효 대지면적 산출 (대지면적 1,250㎡ · 도로 점용부 50㎡)
    MC-->>AG: 유효 대지면적 1,200㎡

    AG->>MC: ⑤ 규모 산정 (1,200㎡ · 건폐율 40% · 용적률 100%)
    MC-->>AG: 건축면적 상한 480㎡ · 연면적 상한 1,200㎡

    AG->>MQ: ⑥ 판정 대조 (공장 허용 여부 · 산출 규모)
    MQ-->>AG: 가능(조건부) · 조건 = 진입도로 폭 확보

    AG-->>SO: 채운 값 반환 (State 반영)
    Note over AG,MC: 순서 판단은 에이전트가 하고 모듈은 받은 값으로 정해진 계산만 수행한다
```
...
#### 5.2.3 모듈 ↔ MCP Tool — 도구 호출과 Observation 반환
...
| 순서 | MCP Tool | 제공 시스템 | 요청 입력 | 반환 결과(Observation) |
|---|---|---|---|---|
| 1 | 지오코딩 | GIS | 도로명 코드 + 건물번호 | PNU · 중심좌표 · 필지 경계 |
| 2 | 토지특성 조회 | GIS | PNU | 지목 · 대지면적 1,250㎡ · 용도지역 계획관리지역 |
| 3 | 규제구역 저촉 조회 | GIS | PNU · 필지 경계 | 저촉 구역 목록 (해당 없음이면 빈 값) |
| 4 | 법령 기준값 조회 | 법령 DB | 용도지역 코드 + 시설물 용도(공장) | 건폐율 40% · 용적률 100% · 허용 여부(조건부) |
| 5 | RAG 근거 검색 | Vector·Graph DB | 판정 항목 + 용도지역 | 근거 조문 인용 · 유사사례 |
| 6 | 3D 매스 생성 (선택) | 3D 엔진 | 건축면적 480㎡ · 연면적 1,200㎡ | 매스 지오메트리 · 토공량 |
...
sequenceDiagram
    ...
    MO->>GIS: ① 지오코딩 (도로명 코드 + 건물번호)
    GIS-->>MO: PNU · 중심좌표 · 필지 경계
...
정리하면 질의 한 건은 Sub의 네 차례 실행 지시, 여섯 개 모듈 기능, 여섯 개 MCP Tool 호출을 거쳐
"계획관리지역 · 건축면적 480㎡ · 연면적 1,200㎡ · 조건부 가능"이라는 결과로 정리된다. 다른
