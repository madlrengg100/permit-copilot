...
## 2026-09-03 — 특정 용도 검토 의견이 LLM 을 아예 부르지 않던 문제

화면의 「검토 의견」이 늘 같은 두 문장 틀("…은 조건부로 건축 가능합니다. 핵심
선행조건은 …이며, 세부 기준은 위 진단 항목에서 확인할 수 있습니다.")로만 나와
LLM 이 응답하지 않는 것처럼 보였다. **LLM 장애가 아니었다** — 키·모델·엔드포인트는
정상이었고(`gemini-flash-lite-latest` 200 / 0.98s), 애초에 호출을 하지 않았다.

`6b45638`(2026-08-06)에서 `render_pending_judgment()` 의 특정 용도 분기가
`_verdict_judgment()`(LLM) 대신 `_concise_verdict_judgment()`(결정적 두 문장)로
고정돼 있었다. 그 뒤로 사용자가 용도를 명시한 질문(창고·단독주택 등)은 전부
결정적 문장만 받았고, `_verdict_judgment()` 는 테스트에서만 불리는 사실상 죽은
코드였다.

- 특정 용도도 다시 `_verdict_judgment()`(LLM, 14s 타임아웃)를 쓴다. 타임아웃·예외·
  빈 응답이면 `_concise_verdict_judgment()` 로 떨어진다(무한 '검토 의견 작성 중' 방지).
- `_verdict_judgment()` 의 검토 용도는 `request.requested_facility` 를 먼저 본다.
  내부 분류 '창고' 대신 화면과 같은 '창고시설' 로 쓰기 위해서다
  (`_concise_verdict_judgment()` 와 같은 규칙).
- 분량은 기존 `_limit_review_length()`(4문장)가 그대로 잡으므로 보고서를 다시
  나열하지는 않는다 — `6b45638` 이 걱정했던 장문 재서술은 이 제한으로 막힌다.

검증: 아산시 음봉면 신수리 100-3 창고 질문을 `/api/chat` 으로 끝까지 돌려
검토 의견이 생산관리지역·건폐율 20%·용적률 80%·최대 건축면적 37㎡ 등 실제 진단값을
읽은 4문장으로 나오는 것을 확인했다(직접 호출 3.8s). 백엔드 테스트 181건 통과.

### 뒤이은 영향 점검에서 같이 고친 것

LLM 을 되살리자 결정적 문장이 덮어 주던 문제 두 가지가 드러나 함께 고쳤다.

**① 검토 의견 시간 예산이 서로 어긋나 있었다(이 증상의 진짜 재발 원인).**
바깥 대기는 14s 인데 호출당 제한은 단일 용도가 SDK 기본 20s×3회, 전체 용도가 50s
였다. gemini 무료 티어가 간헐적으로 내는 503 을 SDK 가 재시도하는 도중 바깥에서
잘려, 첫 질문마다 결정적 폴백 문장만 나왔다(콜드 4회 측정: 2.8s·5.2s·8.1s·20.4s —
20.4s 는 1회 실패 후 재시도 성공). `JUDGMENT_CALL_TIMEOUT=11.0`(호출 1회)과
`JUDGMENT_TOTAL_TIMEOUT=24.0`(바깥 대기) 한 쌍으로 묶어 두 경로가 같이 쓴다.
**한쪽만 고치면 같은 증상이 돌아온다.** 정상 응답은 단일 1.5~8.5s, 전체 1.3~2.0s.

**② `placement_restricted` 필지에서 앞뒤가 모순됐다.** 결정적 문장은 "용도지역상
조건부…만, 기존 건축물 3건이 있어 배치 불가"를 한 문장으로 냈지만, LLM 은 그냥
"조건부 가능합니다"로 시작하고 뒤에 안전장치가 배치 불가 문단을 앞에 붙였다.
협소 대지일 때는 `min_lot_area.note`("법정 최소 대지면적 60㎡에 못 미칩니다.")가
조각으로 먼저 뜨고 그 아래 "조건부로 건축 가능합니다"가 이어졌다. 두 가지를 고쳤다.
- `_verdict_judgment` 프롬프트에 배치 불가 지시를 넣었다(전체 용도 프롬프트와 같은 규칙).
  사유는 데이터에 실제 있는 것만 쓰게 했다 — 기존 건축물이 없는데 '협소 대지'를 붙이던
  환각과, `_strip_internal_field_names` 가 걷어내다 만 "…사유로 가 true여서" 같은
  필드명 유출을 재현해 확인하고 막았다.
- 안전장치의 협소 대지 lead 를 조각 대신 사유+결론 완결 문장으로 바꿨다. 이 lead 는
  전체 용도 경로도 함께 쓰므로 양쪽이 같이 좋아진다.

영향 없음을 확인한 곳: 프런트는 검토 의견 본문을 파싱하지 않는다(마크다운 렌더 +
`options` 버튼만 쓴다). 분할 제안(`land_division` FEASIBLE → `pending_offer='divide'`),
`_limit_review_length`(4문장), `_ensure_query_evidence`·`_ensure_collected_forest_evidence`,
`_model_options_for_diagnosis`, 배지·지도 명령은 그대로다. `server.mjs` 는 SSE 소켓
타임아웃을 걸지 않고(`setTimeout(0)`, `requestTimeout=0`) 백엔드가 2s 하트비트를
보내므로 24s 대기도 연결이 끊기지 않는다. 검증: 배치제한(기존건물·협소)·불가·분할제안
4가지를 `render_pending_judgment` 로 반복 실행, 단일/전체 용도를 `/api/chat` 으로
끝까지 실행, 백엔드 테스트 181건 통과.

## 2026-09-02 — 2D 지도 폴백은 만들었다가 되돌렸다 (재구현 금지)

WebGL 이 막힌 브라우저용 OpenLayers 2D 폴백을 구현해 `b4aa772` 로 올렸다가
사용자 판단으로 되돌렸다. **동작하지 않아서가 아니라 원하지 않아서다** —
3D 한 갈래만 유지하기로 했다.

**다시 만들지 말 것.** 필요해지면 지우고 새로 짜는 대신 되살린다:

```bash
git revert <이 되돌림 커밋>      # 또는
git cherry-pick b4aa772 d830b6c
```

되살릴 때 알아야 할 것(그때 실제로 밟은 함정이다):
- 타일 최대 줌이 레이어마다 다르다 — `midnight` z18, `Base`·`Hybrid`·
  `Satellite` z19. 뭉개면 필지 확대 구간(z19~20)에서 404 가 나고 배경이 빈다.
- `placement: "line"` 금지. 이격선이 세로라 치수 라벨이 90° 누워 뭉개진다.
- `ol` 은 동적 import 여야 한다. 정적이면 주 번들 344KB → 705KB.
- 배경 타일은 `cdn.vworld.kr`(키 불필요) — Referer 도메인 검증을 피한다.
- 데이터는 3D 와 같다. 백엔드는 손댈 필요가 없었다.

당시 검증까지는 끝나 있었다 — WebGL 을 끈 헤드리스 크로미움으로 아산시 음봉면
신수리 100-1 창고 진단을 끝까지 돌려 필지·용도지역·치수·팝업 앵커 표시와
타일 30/30 을 확인했다.

남긴 것: WebGL 원인·조치 안내(`fa8d86b`)와 타일 호스트 https 고정은 그대로다.
WebGL 이 없으면 지도 자리에 원인과 조치 4단계가 뜬다.
...
- 원인은 `explicit_build_question` 분기다. 같은 필지를 다시 물으면
  `emit_card=not same_parcel` 로 `emit_card=False` 가 되어 종합 판정 카드를
  건너뛰는데(중복 방지, 의도된 동작), **모델 버튼도 함께 빠졌다.**
  `_natural_followup_answer()` 산문만 나갔다.
- `_model_options_for_diagnosis()` 자체는 정상이었다. 실제 세션 상태로 돌려 보면
  단독주택형·상가 모델 2건을 계속 돌려준다. 계산은 되는데 방출만 안 됐다.
- 프런트는 필지가 바뀌거나 지도를 클릭하면 기존 모델 버튼을 지운다
  (`App.tsx` 의 `housing:` 필터 3곳). 그래서 재질문에서 다시 내주지 않으면
  가능 판정인데도 모델을 영영 고를 수 없다.
- 같은 문제를 후속질문 경로(`_fu_models`)에서는 이미 처리하고 있었다 — "팔로업은
  카드를 다시 안 띄우지만 모델 제시는 빠지면 안 된다". 이 분기에만 빠져 있었다.
  같은 패턴으로 맞췄다.

진단 절차: 백엔드가 실제로 `options=2` 를 내보내는지 파이프라인을 직접 돌려
확인하고(정상), 세션 기록(`~/.permit-copilot-sessions`)에서 판정·매싱·
`placement_restricted` 게이트를 전부 대조한 뒤(모두 통과), 같은 필지 2회 질문을
재현해 좁혔다. 프런트 번들도 최신본이었다(`dist` 08-26 09:01 > `src` 08:47).

테스트 181건 통과(`test_same_parcel_model_options.py` 2건 신설).

## 2026-08-27 — Codex 중단 작업 회수 및 임상도 전체 무결성 검사 완료

- 로컬 Codex 세션·목표·대기열·프로세스 기록을 대조했다. 미완료 목표와 대기열,
  실행 중인 공간데이터 변환 작업은 없었다.
- 2026-08-23 임상도 변환 후 중단했던 15GB
  `forest_inventory.sqlite`의 SQLite `PRAGMA quick_check`를 읽기 전용으로 재개했다.
- 약 847초 동안 전체 검사를 끝까지 수행했고 결과는 `ok`였다.
- 기존 피처·RTree 각 3,381,067건 일치 및 공간데이터 5개 파일 manifest 검증과 함께,
  임상도 최종 DB의 무결성 확인까지 완료됐다.

## 2026-08-26 (6차) — 용도지역·개별법 관계를 먼저 정하고 룰셋을 돌린다

5차에서 두 축을 무조건 누적 적용했는데 틀렸다. **국토계획법 제76조제5항제3호**를
놓쳤다.
...
아니라 농지법 제32조제1항제3호의 **농업인 주택**이다.

### 2단계로 나눴다
- `resolve_governing(districts)` 신설 — 1단계에서 적용 법령을 정한다.
  `{applied, superseded, basis}`를 돌려준다. 룰셋의 `supersedes: ["농림지역"]`이
  관계를 담는다(농업진흥구역·농업보호구역·임업용산지·공익용산지).
- `evaluate()`와 `use_overview()`는 1단계 결과(`applied`)로만 판정한다.
- 판정 이유가 어느 법으로 판단했는지부터 밝힌다 — "농림지역이지만 농업진흥구역
  이므로 국토계획법 제76조제5항제3호에 따라 개별법이 적용됩니다."
- 카탈로그에 `law.national_land_planning.article_76`(제76조제5항) 등록. 대체가
  일어난 구역의 근거 목록에 이 조문이 먼저 실린다.

### 결과
| 조합 | 이전(5차) | 지금 |
|---|---|---|
| 농림지역 + 농업인 주택 | 조건부(별표 21) | 조건부(별표 21) |
| 농림지역 + 일반 단독주택 | 조건부(별표 21) | 조건부(별표 21) |
| 농림지역·농업진흥구역 + 농업인 주택 | 조건부(별표 21 + 농지법 누적) | **조건부(농지법 제32조만)** |
| 농림지역·농업진흥구역 + 일반 단독주택 | 불가 | 불가(별표 21 근거 소멸) |
| 농림지역·농업진흥구역 + 일반 창고시설 | 불가 | 불가 |

테스트 179건 통과(`SupersessionTest` 5건 신설).

## 2026-08-26 (5차) — 조문에 없는 용도로 예외를 열던 문제

"대천리 241-1에 창고시설은 조건부로 가능합니다" 가 나왔는데 **농지법 제32조에는
'창고시설'이라는 말이 없다.** 조문이 정하는 것은 "농업인 또는 농업법인이 자기가
생산한 농산물을 건조·보관하기 위하여 설치하는 시설"(시행령 제29조제4항제2호가목)
이다. 일반 창고시설 질문을 농업용 시설로 가정해 예외를 열고 있었다.

### 건축법 용도 매칭을 축별로 나눴다
- `district_use_rules.json` 의 허용행위에 `match` 를 뒀다. 농지법 제32조·
  산지관리법 제12조의 27개 항목은 `match: "facility"` — **시설명이 명시돼야**
  예외가 성립하고 건축법 용도만으로는 매칭하지 않는다.
- 국토계획법 시행령 별표 21·22 는 건축법 용도로 열거하므로 용도 매칭을 유지한다
  (예: 별표 21 제1호라목 "제18호가목의 창고").
- 결과: `농업진흥구역 + 창고시설` -> not_allowed,
  `농업진흥구역 + "농산물 건조시설"` -> conditional.
  `농업진흥구역 + 단독주택` -> not_allowed, `+ "농업인 주택"` -> conditional.
- `narrower_paths()` 신설. 불가로 끝내지 않고 "농업용·축산업용·어업용
  시설(제32조제1항제3호)에 해당하면 허용될 수 있다" 는 경로를 이유에 싣는다.

### 용도별 현황표도 행위제한 축이 만든다
- LLM 이 "창고시설이나 교육연구시설 등 예외적으로 허용되는 시설" 이라고 쓴 근거는
  `zone_use_overview` 였다. `building_use_rules.json` 의 간이 판정표가 농림지역
  창고시설을 **조건 없는** 조건부로 내보내고, 별표 21 이 허용하는 단독주택은
  불가로 내보내고 있었다.
- `uses_for_zone(zone, land_districts)` 가 행위제한 축이 아는 구역이면 그 룰셋으로
  현황표를 만든다. 조건부 항목은 허용행위 이름으로 표기한다 — `창고시설` 이 아니라
  `창고(농업·임업·축산업·수산업용)`, `단독주택` 이 아니라
  `단독주택(부지면적 1천㎡ 미만)`.
- 일반 용도가 전부 불가인 구역은 `facility_specific` 이 "그럼 뭐가 되나" 를 답한다.

### 용도 미지정('시설물')이 통째로 불가가 되던 문제
- `시설물` 은 용도 미지정 표식이지 건축물 용도가 아니라, 행위제한 열거에서 찾을 수
  없어 금지로 떨어졌다. overview 로 판정하도록 바꿨다. 농업진흥구역에서는
  "일반 건축물 용도로는 건축할 수 없고 개별법이 열거한 시설에 해당할 때만
  허용됩니다: 농업인 주택·어업인 주택 · 농업용·축산업용·어업용 시설 …" 로 답한다.

테스트 174건 통과(`test_district_use_match_policy.py` 7건 신설).

## 2026-08-26 (4차) — 경산시·계양구·예산군 지구단위계획 수집

3차에서 "원본 URL이 없어 못 한다"고 남긴 세 지자체를 토지이음 고시 목록에서
직접 찾아 수집했다. **6개 지자체 32개 지구, 1,119청크**(3차 10지구 527청크에서
확대). 원본 152건 299MB, 통합 색인 11,249청크.

### 고시 목록 자동 탐색
- `collect_district_plans.py --discover <시군구…>` 신설. 토지이음
  `gvGosiList.jsp` 를 훑어 그 지자체의 지구단위계획 결정고시를 찾아
  `district_plan_sources.json` 에 `discovered: true` 로 등록한 뒤 수집한다.
  이름을 생략하면 `sources` 가 비어 있는 지자체 전부가 대상이다.
- **질의 문자열도 euc-kr 로 인코딩해야 한다.** UTF-8 로 보내면 0건이 나온다.
  첨부 다운로드와 같은 함정이다.
- 제목에 `지구단위계획` 이 있어도 `열람·공고·폐지·실효·취소` 는 제외한다. 계획이
  아니라 절차 고시라 원문에 계획 내용이 없다. 계획명은 제목 괄호 안에서 뽑는다.
- 탐색 결과: 예산군 23건 · 경산시 38건 · 계양구 26건 중 각 12건씩 수집(기본
  `--discover-limit 12`).

### OCR 은 문서형만
- 스캔본 24건을 통째로 OCR 하니 52페이지에 20분 넘게 한 건도 끝나지 않았다.
  파일명이 `도면·결정도·종합도·총괄도·계획도·지형도` 인 14건(29p)은 지도라
  텍스트 회수율이 낮고 페이지당 수 분이 걸린다.
- 문서형 10건(24p)만 태워 몇 분에 끝냈다. 도면의 획지 수치는 결정조서·시행지침
  표에서 이미 나온다.
...
세 지자체 모두 `scope="district_plan"` 검색으로 회수된다. 예산 예산지구,
경산 압량지구, 계양 효성 지구단위계획 등.

### 남은 것
- 획지·PNU 매핑은 여전히 미완이다. 지구단위계획 수치는 근거 표시용이다.
- 도면 14건은 OCR 하지 않았다. 필요해지면 파일 단위로 `--ocr` 를 지정한다.

## 2026-08-26 (3차) — 지구단위계획 수집·정제·청킹

### 파이프라인 3단계 신설
- `collect_district_plans.py` 신설. 고시 페이지에서 첨부 원문을 받아
  `data/source/district_plans/<시군구>/<계획명>/` 에 저장한다.
- 정제는 기존 `parse_district_plan_documents.py`, 청킹은 신설
  `build_district_plan_chunks.py`. 결과를 `build_ordinance_index.py` 가 조례·법령과
  **같은 TF-IDF 색인**에 합친다. 한 번의 검색으로 셋을 함께 회수한다.
- **11개 지구 중 10곳, 527청크**(본문 423 · 표 104). 원본 38건 156MB.
  `data/source/`·`data/processed/` 는 `.gitignore` 대상이라 청크 JSON만 저장소에
  들어간다. 색인은 10,130 -> **10,657청크**.

### 수집에서 막혔던 것
- **토지이음은 `gosi=Y` 히든 필드와 euc-kr 인코딩이 둘 다 맞아야 한다.** 하나라도
  틀리면 HTTP 200 에 "첨부파일이 없습니다" 스크립트가 온다. 빈 고시와 구분되지
  않아 처음엔 음성군 7개 지구 중 4곳을 "첨부 없음"으로 오판했다.
- 아산시 CMS 첨부 링크는 확장자가 없는 `download.php?...&uid=` 다. 확장자로만
  긁으면 본문과 무관한 사이드바 문서(주민참여예산제 조례)를 가져온다. 실제
  파일명은 앞의 `preview.php?file_nm=` 에 있다.
- 비브라우저 User-Agent 에 응답 없이 연결을 끊는 CMS 가 있다. 브라우저 UA +
  Referer + 재시도로 해결. 받은 바이트는 매직 넘버로 검사한다.

### 정제에서 막혔던 것
- HWP 파서가 없었다. `pyhwp` 와 `six` 를 `requirements.txt` 에 추가했다.
- `hwp5html` 이 큰 시행지침에서 3분을 넘겨 배치가 멈췄다. 표 추출 180초 타임아웃
  후 본문만 살리고, 파일 단위 예외도 잡는다.
- OCR 이 페이지 하나 타임아웃에 문서 전체를 버렸다(90초). 타임아웃 300초·재시도
  2회로 바꾸고 실패 페이지는 `OCR_FAILED` 로 남긴다. 스캔본 7건 중 재실행으로
  전부 복구, 26페이지 OCR 완료.

### 표를 함께 청킹해야 한다
- 획지별 건폐율·용적률·최고층수는 대부분 표에만 있고 `hwp5txt` 본문에는 `<표>`
  표시만 남는다. 아산 탕정 결정조서는 본문 864자 / 표 68,545자다. 표를 빼면 핵심
  수치가 통째로 색인에서 빠져 첫 청킹에서 391청크에 그쳤다. 표 청킹을 넣어 495 ->
  OCR 반영 후 527청크.

### scope 격리와 화면 연결
- `ordinance_index.search(scope="district_plan")` 신설. `scope="ordinance"` 는
  지구단위계획을 제외한다 — 조례 근거 섹션에 다른 지구의 시행지침이 섞이면 안 된다.
- `run_prediagnosis` 가 `district_plan_passages` 로 시행지침·결정조서 원문 발췌를
  붙이고, 기존 링크 목록과 함께 '지구단위계획 근거자료' 섹션에 렌더링한다.
  획지·PNU 매핑 전이므로 "원문 발췌 — 획지 확정 전 참고"로 표시하고 수치 판정에는
  쓰지 않는다.

### 남은 것
- 경산시·계양구는 `district_plan_sources.json` 에 `sources` 가 비어 있다.
  예산군은 목록에 없다. 고시 페이지 URL 을 채우면 수집기가 그대로 받는다.
- 획지·PNU 매핑은 아직이다. 그전까지 지구단위계획 수치는 근거 표시용이다.

## 2026-08-26 (2차) — 법령 데이터 복구와 행위제한 축 신설

### 조례 근거가 화면에서 통째로 사라져 있던 원인 — 색인 파일 유실
- `ordinance_index.npz` 등 색인 3종이 `.gitignore` 대상인데 기동 시 재생성하는
  코드도 systemd 설정도 없다. 서버 이관 후 아무도 다시 만들지 않아
  `ordinance_index.available()` 이 False 였고, `ordinance_evidence` 가 조용히 빈
  배열이 되면서 '지자체 조례 근거자료' 섹션 헤더 자체가 생성되지 않았다.
  프런트의 접기/펼치기 규칙은 정상이었다 — 감쌀 섹션이 없었을 뿐이다.
- 전국 도시·군계획 조례 XML 196건을 재수집해 `.cache_ordin` 을 복원하고 색인을
  다시 만들었다. **2,327 -> 10,130청크**(도시계획조례 7,118건 신규).
  화면에서 "해당 조례가 없다"와 구분되지 않는 실패라 문서에 진단 절차를 남겼다.

### 법령 corpus의 49%가 조문 제목만 담고 있던 문제
- 국가법령정보센터 XML은 항·호가 있는 조문의 본문을 `<항내용>`·`<호내용>`·
  `<목내용>` 에 나눠 넣고 `<조문내용>` 에는 제목 줄만 둔다. 수집기가 `<조문내용>`
  만 읽어 1,748청크 중 857개가 제목뿐이었다.
- 농지법 제32조는 "제32조(용도구역에서의 행위 제한)" 19자가 전부였다. 진흥구역
  허용행위 9호와 보호구역 3호가 통째로 없었다. 시스템이 근거로 인용하는 제34조도
  17자였다. 편·장·절 제목(`조문여부='전문'`)이 조문번호를 공유해 "제2절 농지의
  전용"이 제34조 청크로 섞이는 문제도 함께 고쳤다.
- 재수집 결과 **1,748 -> 2,415청크**, 제목만 남은 청크 0건, 본문 중앙값 27자 ->
  421자. XML 원본과 대조해 미수집 354조를 전수 확인했고 벌칙·과태료·양벌규정·
  위원회 운영·삭제조문뿐이었다.

### 조례 용적률의 천단위 쉼표를 못 읽어 176개 수치가 비어 있던 문제
- 파서 정규식이 `(\d{1,4})` 라 "1,500퍼센트"의 쉼표에서 끊겼다. 900은 잡히고
  1,500은 null이 되는 식으로 상업지역 용적률이 전국적으로 누락됐다.
- 재파싱 결과 **73개 지자체 176개 수치 회복**, 손실 0. ground truth 대조 정확도
  93.5% -> 95.0%, 불일치는 계속 0건.
- 부여군은 조례가 실제 개정돼(시행 20251218 -> 20260825) 7개 zone이 함께 바뀌었다.

### 용도구역 행위제한 축 신설 — 하드코딩 분기를 룰셋으로
- `zoning.py` 에 `agricultural_house and zone == "농림지역"` 조건 분기와 산문
  `reason` 으로 박혀 있던 판정을 `district_use_rules.json` + `district_use.py` 로
  옮겼다. 농업인 주택만이 아니라 각 구역의 허용행위를 조문 단위로 전부 담는다.
- 두 종류의 구역을 같은 스키마로 담는다. 용도지역 축(농림지역 별표 21,
  자연환경보전지역 별표 22)과 개별법 용도구역 축(농업진흥구역·보호구역 농지법
  제32조·시행령 제29·30조, 임업용·공익용산지 산지관리법 제12조). 42개 허용행위.
- 구역 판별은 공간레이어 코드(UEA110/UEA120/UFM110/UFM120)를 이름보다 먼저 본다.
  이 코드는 이미 들어오고 있었는데 표시 문구를 만드는 데만 쓰고 있었다.
- 두 축이 겹치면 누적 적용하고 한 곳이라도 금지면 금지다. 예외 열거에 해당해도
  대통령령·부령 요건이 남아 판정은 conditional 이 상한이다.
- **원문 대조로 확인된 오류 정정: 농림지역의 일반 단독주택은 불가가 아니다.**
  별표 21 제1호가목이 부지면적 1천㎡ 미만 단독주택을 조례 없이 건축할 수 있는
  건축물로 열거한다. 농업인 자격은 요건이 아니다. `building_use_rules.json` 의
  단독주택에 농림지역이 빠져 불가로 떨어졌고, 농업인 주택만 분기로 되살리고 있었다.
  이 판정이 `not_allowed` -> `conditional` 로 바뀐다.
- 별표 22는 자연환경보전지역 단독주택을 "농지법 제32조제1항제3호에 따른 농업인
  주택·어업인 주택"으로만 허용한다. 용도지역 축이 개별법 조문을 직접 인용하는
  구조라 두 축을 한 파일에 둔 근거가 된다.

### 판정 근거 조문이 화면에 뜨지 않던 순환 구조
- 전국 법령 RAG 질의가 `permit_rules` 가 선택한 절차 단계명·근거로만 만들어져
  순환했다. 카탈로그에 없는 조문은 질의에 안 들어가고 -> 검색되지 않고 -> 계속
  카탈로그 밖에 남는다. 농지법 제32조가 corpus에 있는데도 한 번도 안 뜨던 원인이다.
- 카탈로그에 행위제한 참조 7건을 등록하고(제32조, 시행령 제29·30조, 산지관리법
  제12조, 국토계획법 시행령 제71조·별표 21·별표 22), `_merge_permit_legal_evidence`
  가 이 근거를 **절차 근거보다 먼저** 싣도록 했다. 질의에도 행위제한 축이 판정한
  구역명·조문을 넣어 고리를 끊었다.
- 답변에 '용도구역 행위제한' 섹션을 추가해 해당 구역·허용 근거 조문·충족해야 할
  요건을 표시한다.
- 대천리 241-1(농림지역·답, 농업인 주택) 검증: 농림지역 별표 21 제1호가목과
  농업진흥구역 농지법 제32조제1항제3호가 함께 매치되고 요건 6건, 근거 조문 4건이
  화면 목록에 실린다.

### 남은 것
- 지구단위계획 청킹은 원문이 없어 하지 못했다. `parse_district_plan_documents.py`
  는 `backend/data/source/district_plans/` 의 로컬 PDF·HWP를 요구하는데 디렉터리
  자체가 없고 자동 다운로더도 없다. `district_plan_sources.json` 에 등록된 5개
  지자체(경산·아산·음성·계양·영천)도 모두 `status: collecting` 이고 예산군은 목록에
  없다. 각 지자체 고시 페이지에서 원문을 받아 두는 선행 작업이 필요하다.
