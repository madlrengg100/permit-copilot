...
## 2026-09-02 — WebGL 이 막힌 브라우저용 2D 지도 폴백

사용자 환경의 WebGL 이 **회사 정책으로** 꺼져 있었다. 브라우저 설정으로 풀 수
없으니 3D 는 포기해야 하는데, 그렇다고 지도를 못 쓰게 둘 수는 없다.

- `frontend/src/lib/mapSurface.ts` — 호출부가 보는 유일한 표면(13개 메서드).
  3D(`MapBridge`)와 2D(`Map2DBridge`)가 함께 구현한다. `App.tsx`·`MapCanvas.tsx`·
  `MapCompass.tsx` 는 이제 구체 구현을 모른다.
- `frontend/src/lib/map2dBridge.ts` — OpenLayers(Canvas 2D) 구현. 3D 실패 시
  자동 전환한다. 무엇이 빠졌는지는 `capabilities` 로 알리고 화면이 그 값으로
  버튼을 감춘다(2D/3D 전환·높이 측정).
- **데이터는 3D 와 완전히 같은 것을 쓴다.** 배경만 VWorld 타일이고 필지·용도지역·
  경사도는 기존 `/api/parcels`·`/api/zoning-area`·`/api/parcel-at` 응답이다.
  백엔드는 한 줄도 고치지 않았고, 판정과 지도가 어긋날 여지가 없다.
- 2D 에서 되는 것: 필지 강조, 용도지역·규제 조각, 치수선·면적 라벨, 경사도 격자
  (백엔드가 셀 도형과 경사도를 함께 주므로 색으로 깐다), 지적도, 클릭 필지 선택,
  결과 팝업 앵커, 거리·면적 측정(OpenLayers Draw, 타원체 계산).
  `extrude_mass` 는 높이를 못 세우는 대신 **건축면적 평면 윤곽**을 그린다.
- 2D 에서 빠지는 것: 건물 3D 매싱(`show_lod1`·`show_housing_model`), 절토·성토,
  높이 측정. 조용히 넘기지 않고 콘솔에 남긴다.

**함정 세 개를 실제로 밟고 고쳤다.**

1. **타일 최대 줌이 레이어마다 다르다.** `midnight` 은 z18, `Base`·`Hybrid`·
   `Satellite` 는 z19 까지다. source `maxZoom` 을 19 로 뭉개 두면 필지를 확대한
   화면(z19~20)에서 404 가 나고 **배경이 빈 채로** 남는다. 하필 실제로 쓰는
   줌 구간이라 놓치기 쉽다. `TILE_MAX_ZOOM` 으로 층별 실측값을 박았다.
2. **`placement: "line"` 금지.** 이격선은 대개 세로라 라벨이 90° 누워 세로쓰기처럼
   뭉개진다. 선 중점에 가로로 놓고, 치수·분할 레이어는 `declutter` 로 겹침을
   떨어뜨린다.
3. **`ol` 은 지연 로딩해야 한다.** 정적 import 로 넣으면 주 번들이 344KB →
   705KB 로 뛴다. WebGL 되는 사용자는 2D 코드를 받을 이유가 없다.
   동적 import 로 갈라 주 번들 347KB + 필요할 때만 358KB.

배경 타일은 `cdn.vworld.kr` 을 쓴다 — **키가 필요 없어** Referer 도메인 검증을
타지 않는다. `api.vworld.kr/req/wmts` 는 키·등록 도메인이 맞아야 하고, 그건 서버
이전마다 어긋나는 알려진 함정이다(`docs/SERVER-MIGRATION.md`).

검증: WebGL 을 끈 헤드리스 크로미움으로 실제 진단을 끝까지 돌렸다(아산시 음봉면
신수리 100-1, 창고). 타일 30/30 정상, 4xx 없음, 필지·용도지역·치수·결과 팝업
앵커까지 그려진다. WebGL 이 있는 경우 3D 로 가는 경로도 그대로다 — SwiftShader
성공률은 원래부터 5회 중 1~2회로 불안정하고, 실패하면 이제 에러 대신 2D 로
내려간다. 백엔드 테스트 181건 통과.
