...
## 2026-08-03
### 도로접촉선: 안쪽 밀기 대신 '경계선 위로 띄우기'로 겹침 해소
- 직전 커밋은 접촉선을 필지 안쪽 0.6m로 들였는데, 실제 접한 변에서 너무 떨어져
  보였다(사용자 피드백). 좌표는 경계선 그대로 두고, show_dimensions 세그먼트에
  lift_m(지면에서 띄우는 높이)을 추가해 도로접촉선만 지면부착 대신 0.6m 띄워
  그린다. 지면부착 청록 경계선 위로 떠서 카메라 시점에서 두 줄이 분리돼 보이고,
  자주색이 경계선을 덮지 않는다(mapBridge showDimensions: lift_m 있으면 각 정점을
  terrainHeight+lift 로 올리고 clampToGround=false).

### 도로접촉선 이중선(경계선 덮음) 해소 + 접촉길이 라벨 뒤바뀜 수정
- 도로접촉선(자주)이 필지 경계선(청록)과 좌표가 같아 지면부착 선 둘이 겹쳐, 자주색이
  청록 경계를 덮으며 한 줄이 이중선처럼 보였다(경계가 안 보임). 접촉선을 필지 안쪽으로
  약 0.6m 들여(parallel_offset, 필지 내부 방향 선택) 경계선과 겹치지 않는 나란한 선으로
  그린다. 굵기도 9→7. 라벨의 접촉길이는 원래 값 유지.
- 접촉길이 라벨이 인덱스 순서로 붙어 2.3m↔36.5m가 뒤바뀌던 버그도 수정 — 각 접촉선
  조각을 그 길이에 가장 가까운 도로에 매칭해 라벨링한다. 308-1 검증(0.60m 이격·라벨
  정상)·110 테스트 통과.
- 함께: 어제 넣은 도로 후퇴선(편입 안쪽 경계 표시)이 접촉선과 같은 변에 겹쳐 이중을
  더했기에 제거하고, 후퇴폭은 '도로 편입 약 N㎡ · 후퇴 N.Nm' 라벨로 합쳤다.
