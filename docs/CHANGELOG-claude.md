...
### '선택 필지에 …' 질문이 엉뚱한 지번을 답하던 문제
- 주소를 **입력해서** 다른 필지를 물으면 카드·지도는 그 필지로 바뀌지만, 프런트의
  `selectedLocation` 은 지도 클릭으로만 갱신돼 이전 필지에 머문다. 다음 턴에 프런트가
  그 옛 값을 보내면 `main.py` 가 `from_mouse = (보낸 PNU != 진단 PNU)` 로 **새 지도 클릭**
  으로 해석해 이전 필지 진단을 복원한다. 사용자는 그 필지를 다시 누른 적이 없다.
- 실제 세션 기록(`~/.permit-copilot-sessions`)에서 확인했다 — 질문 이력은 두리 100 을
  물었는데 `diagnosis.parcel`·`selected_parcel` 이 모두 두리 111 이었다.
- 백엔드는 진단 직후 `set_selected_parcel` 로 이미 동기화하고 있었으나 다음 턴에 프런트
  값이 덮어썼다. `show_panel` 에 `lon`/`lat` 을 실어(기존엔 pnu·address 만 있어 선택
  상태를 만들 수 없었다) 프런트가 **분석 중인 필지로 선택을 맞추도록** 했다.
- 책임 분리 원칙대로 프런트에는 판정 로직·정규식을 넣지 않았다(백엔드 값 추종만).
- 앞선 '111 필지를 선택했습니다' 잔류 문구 수정은 표시만의 문제였고, 이건 답변 대상이
  틀리던 별개 원인이다.

### 필지를 되돌아 선택하면 이전 필지 지번이 안내에 남던 문제
- 100 진단 → 111 클릭(질문은 안 함) → 100 재클릭 하면 "…두리 111 필지를 선택했습니다"
  가 "…두리 100 필지입니다(현재 분석 중인 필지)" 위에 그대로 남아, 지금 보는 필지와
  다른 지번이 최신 안내처럼 보였다.
- 원인은 `App.tsx` 의 sameParcel 분기다. `activePnu` 는 진단 중인 필지(panel.pnu)라
  111 을 눌러도 질문 전에는 100 그대로다. 그래서 100 재클릭이 sameParcel 로 잡히는데,
  그 분기가 status('현재 분석 중인 필지')만 걷어내고 111 때 남긴 assistant 안내는
  지우지 않았다.
- 분석 중인 필지로 되돌아오면 그 사이에 남긴 '무슨 건물' 안내도 함께 지운다.
  (해당 분기는 이미 `parcelPromptPendingRef=false` 로 그 안내를 무효 처리하고 있었다)
...
- 용도를 특정한 질문(예: '단독주택')은 LLM 을 거치지 않고 결정적 템플릿으로 검토 의견을
  ...
  한 화면에서 "신축을 배치할 수 없습니다" 와 "조건부로 건축 가능합니다" 가 함께 나왔다.
  **할루시네이션이 아니라 템플릿 결함이다.**
- `_concise_verdict_judgment` 가 `placement_restricted` 를 보지 않고 verdict 만으로 판단했다.
  배치 불가를 결론에 반영한다. 안전장치(`render_pending_judgment`)는 앞 80자에서만 '배치' 를
  찾아 긴 문장에서 같은 안내를 또 붙였다 — 문단 전체를 보도록 고쳤다.
- 사유 추측도 함께 고쳤다. `map_presentation.verdict == not_allowed` 까지 '실질 배치 불가' 로
  묶는 바람에 **21,620㎡ 필지를 '협소 대지'로 설명**했다. `placement_restricted` 만 보고,
  사유를 특정할 수 없으면 지어내지 않는다(용도 자체 불가는 use_restriction.reason 분기).
- `tools/textfmt.josa()` 신설 — 한글 유니코드 받침 판정으로 이/가·은/는·을/를·과/와 선택.
  따옴표로 끝나면('농림지역') 안쪽 글자로 판단한다. "…교육연구시설 등"+"가" → '등가' 가
  '등이' 로, 리터럴 플레이스홀더 `은(는)` 11곳, `입니다입니다` 중복을 정리했다.
- **남은 과제**: all-uses(용도 미지정) 경로의 LLM 프롬프트가 지시 20개 넘게 부풀어 있고
  실측 17초로 14초 타임아웃을 넘겨 결정적 fallback 으로 떨어지는 일이 잦다. 프롬프트 축소는
  출력 품질이 바뀔 수 있어 여러 필지 샘플을 확보한 뒤 비교하며 별도로 진행한다.
