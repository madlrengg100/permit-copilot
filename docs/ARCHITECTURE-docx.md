...
> **이 파일은 docx 변환용이다.** 내용은 `ARCHITECTURE.md` 와 같고, 다이어그램만
> Word 에서 깨지지 않는 고정폭 도해·표로 바꿨다. 웹에서 볼 때는 원본을 쓸 것.
>
...
│  ChatPanel        MapCanvas         MapBridge      ResultPanel   │
...
└──────┬──────────────────┬───────────────────┬────────────────────┘
       ▼                  ▼                   ▼
  LLM (Anthropic     VWorld OpenAPI      ordinances.json
  / OpenAI 호환)     지오코더·2D데이터     시행령 + 지자체 조례
```
...
② 사전진단 ──► LLM       "주소·용도 추출"  (submit_request 강제)
   ...
   지도제어 ──► [clear_mass, fly_to, highlight_parcel,
                 extrude_mass, show_panel]      → map_commands
                                                ▼
                        MapBridge.execute() — 지도 묘화

③ Orchestrator ──► LLM   "결과를 종합해 답변"
               ◄──       최종 답변 텍스트       → message → done
```
...
10개)으로 정규화한다. 용도가 없으면 단일 용도를 추측하지 않고 `시설물`,
`inferred=true`로 표시해 지원 용도 전체를 검토한다.
...
`resolve_limits(zone, jurisdiction)` 의 판정 순서:

| 조건 | 결과 |
|---|---|
| 시행령 용도지역 목록에 없음 | `found=false` — 판정 불가 |
| 지자체 조례 데이터 없음 | 법정 상한 (`source=statutory`) |
| 조례에 해당 용도지역 항목 없음 | 법정 상한 + 미규정 사유 기록 |
| 항목은 있으나 건폐율·용적률이 모두 `null` | 법정 상한 + 미규정 사유 기록 — **조례 적용으로 취급하지 않는다** |
| 조례가 값을 규정 | 조례값으로 덮어씀 (`source=ordinance`). `null` 인 항목만 법정값 유지 |
...
- 조례 조문 벡터색인: 7,585청크·193관할(근거 검색용, 수치 판정 미사용)
...
법률·조례 수집·청킹·검색의 상세 단일 원본은
`docs/LEGAL-ORDINANCE-INDEX.md`다. 정형 JSON이 수치를 결정하고 TF-IDF 조문
색인은 관할 근거 인용에만 사용한다.
