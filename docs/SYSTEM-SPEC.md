...
flowchart TB
    subgraph FE["프론트엔드 · React + Vite · :5173"]
        CP["ChatPanel<br/>질의 입력 · 대화 로그 · 상시 고지"]
        MC["MapCanvas<br/>VWorld 3D 부트스트랩 · 2D/3D 전환"]
        MB["MapBridge<br/>지도 명령 실행 · 카메라 · 묘화"]
        RP["ResultPanel + 범례<br/>건물 위 부착 · 용도지역 구분"]
    end

    subgraph BE["백엔드 · FastAPI · :8000"]
        API["main.py<br/>SSE 스트리밍 · 토큰 인증 · 오류 변환"]
        ORCH["Orchestrator<br/>LLM 도구 루프 (최대 8턴)"]
        PRE["사전진단 에이전트<br/>결정적 파이프라인 · LLM 1회"]
        MAPC["지도제어 에이전트<br/>LLM 미사용"]
        TOOLS["도구 계층<br/>vworld · zoning · ordinance<br/>jimok · massing · footprint"]
    end

    subgraph EXT["외부"]
        LLM["LLM<br/>Anthropic / OpenAI 호환"]
        VW["VWorld OpenAPI<br/>지오코더 · 검색 · 2D데이터"]
        VW3D["VWorld 3D 엔진"]
...
sequenceDiagram
    ...
    participant V as VWorld
    participant M as 지도제어

    U->>FE: "두리 100에 공장 지을 수 있어?"
    FE->>O: POST /api/chat (SSE 개시)

    O->>L: ① 도구 선택 (SYSTEM + TOOLS)
    L-->>O: prediagnose(query=…)
    O-->>FE: tool_start

    O->>P: run_prediagnosis()
    P->>L: ② 주소·용도 추출 (submit_request 강제)
    L-->>P: {address, building_use, inferred}

    Note over P,V: 이하 LLM 미사용 — 결정적 파이프라인
    P->>V: geocode → get_parcel → get_land_use
    P->>V: get_zone_shares (필지 폴리곤 교차)
    P->>P: jimok.classify → zoning.lookup → massing.calc
    P-->>O: diagnosis

    O-->>FE: diagnosis_step ×6~7
    O-->>FE: diagnosis (전체)
    O->>M: build_map_commands()
    M-->>O: 명령 배열
    O-->>FE: map_commands
...
숨김/복원)를 `control` 필드로 반환한다. 빠른 규칙이 즉시 처리하고, 규칙이 못 잡은 동의어·구어·오타만
제미나이가 의미로 해석한다. 주소와 함께 온 찾기/이동 표현("어디야"·"○○ 찾을 수 있어")은 종합진단이
아니라 그 필지로 이동하며(`move_to_parcel`, 표준+학습 표현 판정 `_requests_move_phrase()`,
건축·모델 요청은 제외), "○○라고 하면 ○○하라는 뜻이야"처럼 의미를 가르치는 교육 문장은
`_is_teaching_statement()`로 개념 답변 경로에서 제외해 학습 훅에 도달시킨다. 학습된 표현은 제어
...
(가능 판정일 때만), 우수·오수 '개념' 배수로(파랑), 치수(가로·세로·높이)·면적(대지·건축)을
세그먼트로 만든다. 인접에 공공 배수처가 없어 배수로가 사유지를 지나면 최근접 공공 배수처까지
**침범 경로**를 그리되, 통과 필지의 소유구분(토지소유정보)·건물 유무(건축물대장)로 라벨을
3갈래(사유 확인/사유추정/국공유 통과가능)로 나눈다. 라벨은 방류 목적지(구거/도로/하천,
`encroachment.outlet`)를 함께 표시하고, 침범 빨강과 겹치지 않게 그때 건축선은 보라(#7E57C2)로
바뀐다. `overlay_command(diagnosis, kinds)`는 요청한 선만 골라 얹고,
...
| | `get_ledger_area_m2(pnu)` | 토지대장 공부면적 `lndpclAr`(NED 토지특성 `getLandCharacteristics`), 실패 시 None |
...
| 연속지적도에 **면적 필드 없음** | 법적 기준인 토지대장 공부면적(`get_ledger_area_m2`→`lndpclAr`)을 `area_m2` 로 우선 사용, 실패 시 경계에서 pyproj `Geod` 측지면적으로 폴백 + `area_source` 로 출처 명시. 걸침 조각 면적도 공부면적에 안분 |
...
flowchart TD
    A[용도지역] --> B{시행령 목록에 있나}
    B -->|아니오| X["found=false · 판정 불가"]
    B -->|예| C{지자체 식별됨}
    C -->|아니오| D{비도시지역인가}
    D -->|예| Y["found=false<br/>requires_ordinance=true<br/>법정 대체 계산 안 함"]
    D -->|아니오| E["법정 상한 적용<br/>source=statutory"]
    C -->|예| F{조례에 해당 지역 규정}
    F -->|"항목 없음/null뿐"| G{비도시지역인가}
    G -->|예| Y
    G -->|아니오| H["법정 상한 + 미규정 사유"]
    F -->|예| I["조례값 적용<br/>source=ordinance<br/>null 항목만 법정 유지"]
```
...
- 면적은 **토지대장 공부면적**(`lndpclAr`)을 우선 사용하며 토지이음과 일치한다. 조회 실패 시에만
  지적도 경계의 측지 계산값으로 폴백한다(출처는 `area_source`).
...
| 항목 | 내용 |
|---|---|
| 세션 저장소 | 프로세스 메모리 + `/home/jh12535320/.permit-copilot-sessions` 파일 스냅샷. 재시작 시 PNU별 진단·대화 상태 복원 |
