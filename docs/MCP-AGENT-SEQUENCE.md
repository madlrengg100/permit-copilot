...
**구성 — Sub 오케스트레이터 3개 · 에이전트 4개 · 모듈**

| ② Sub 오케스트레이터 | ③ 에이전트 | ④ 모듈 |
|---|---|---|
| **지도 · 공간 Sub**<br>State 판단 · Tool 제어(MCP) | **지도제어 에이전트** | 의도 분류 · 파라미터 추출<br>GIS 명령 생성 · 실행<br>결과 해석 · 토지특성 통합 |
| | **공간분석 비전 에이전트** | 항공영상 객체 식별<br>공간정보 연계 · 검증<br>필지 이용현황 도출 |
| **3D 시뮬레이션 Sub**<br>State 판단 · Tool 제어(MCP) | **3D 시뮬레이션 에이전트** | 경사도 · 토공량 분석<br>지형 · 건물 자동 생성<br>V-World 연계 시각화 |
| **사전진단 Sub**<br>State 판단 · Tool 제어(MCP) | **사전진단 추론 에이전트** | RAG 근거 검색 · 판정<br>민원세트 생성<br>보고서 · 무결성 검증<br>조건부 대안(2차년도) |
...
**LLM · RAG 는 공용이다.** Sub 마다 따로 두지 않고 하나를 함께 호출한다.

**ReAct 루프** — 에이전트는 한 번에 끝내지 않고 필요한 값이 다 모일 때까지 돈다.

```
Thought(실행계획) → Action(도구 호출) → Observation(결과 관찰) → 재판단 반복
```

**오르내리는 것** — 내려갈 때는 `요청 · MCP 도구 호출`, 올라올 때는 `결과 반환(Observation)` 이다.
...
| **Graph DB** | **Fuseki** (RDF/SPARQL) | **법령 산정방식 룰셋(규칙) 원천** — 법령 간 연계·적용관계를 온톨로지 지식그래프(법령/조문/적용대상 3계층)로 담아, 건폐율·용적률·이격·부담금 등 **산정 규칙(룰셋)을 도출·구성**한다 | **Rule Engine(룰셋 생성)**, RAG Graph 검색, 저촉 판정(§4.4) |
| **Vector DB** | **Milvus** | **법령 조문 + 판례·민원 유사사례** 임베딩 청크 — **법령 검색**과 **유사사례 검색** 둘 다에 쓴다 | RAG Vector 검색(법령·유사사례) |
...
|---|---|---|
| 담는 형태 | **정형** — 조문 번호, 기준값, 적용 조건 | **청크 + 임베딩** — 법령 조문 본문, 판례·민원 사례 |
| 답하는 질문 | *"계획관리지역 건폐율 상한은?"* | *"관련 법령 조문은?(법령 검색)"* · *"비슷한 사례는 어떻게 판단했나?(유사사례)"* |
...
**RAG 는 Hybrid RAG 다** — `RDB → Graph DB → Vector DB` 같은 **직렬 구조가 아니다.**
RAG 근거검색 모듈이 **Graph DB(법령 지식그래프)와 Vector DB(임베딩 청크)를 함께 조회**해
그래프 확장 + 벡터 유사검색을 결합(하이브리드)하고 리랭킹한다. RDB 는 이 둘의 원천으로,
`RDB → Graph DB` 는 **적재(build-time)** 관계(법령 개정 시에만)이고, `Graph DB·Vector DB → RAG`
는 **조회(run-time)** 관계(질의마다)다 — 방향과 시점이 다르다.
...
### 1.4 온톨로지 법령 지식그래프 구조

Graph DB 에 들어가는 것의 구조다. 법령·조례와 용도지역, 판단 규칙을 **하나의 온톨로지로 통합**해
인허가 판정의 **법적 근거 추적성**과 **Rule Engine · RAG 검색**에 직결시킨다.

| 덩어리 | 무엇 | 아래에 딸린 것 |
|---|---|---|
| **법령 (Law)** | 법률 · 시행령 · 시행규칙 | **항 · 호 · 목 (Unit)**, **별표 · 부칙**(시설물별 행위기준) |
| **조문 (Article)** | 법령을 이루는 개별 조항 | **조례** |
| **공간 (Zone)** | 용도지역 · 지구 · 구역 | **보호 · 규제구역**, **건폐율 · 용적률**, **행위제한**(시설물 용도별 행위) |

**덩어리를 잇는 관계** — 이것이 이 그래프의 핵심이다. 조문을 아무리 많이 모아도
관계가 없으면 우선순위를 판정할 수 없다.

| 관계 | 무엇을 잇나 | 어디에 쓰나 |
|---|---|---|
| **Zone ↔ 법령** | 이 용도지역에 어느 법이 걸리는가 | 판정의 출발점 |
| **인용** | 이 조문이 어느 조문을 끌어다 쓰는가 | RAG Graph 확장(§4.5 ②) |
| **의제** | 하나 받으면 무엇까지 받은 것으로 보는가 | 민원 절차 축소(§4.2 3단계) |
| **조건** | 언제 적용되고 언제 아닌가 | Rule Engine 조건 판단(§4.6) |

**무결성 강제 규칙(SHACL)** — 데이터가 정해진 모양을 지키는지 자동 검사하는 규칙이다.
예: *"조문에는 반드시 소속 법령이 있어야 한다"*. 통과했다는 것은 **빠지거나 잘못 연결된
항목이 없다**는 뜻이다.

> 이 구조 덕분에 판정 근거를 **조문 단위까지 되짚을 수 있다.**
> "이 판정은 어느 법 몇 조 때문인가"에 답할 수 있다는 뜻이다.
...
예: *"충북 음성 두성리 100에 공장 지을 수 있어?"* — 개발 검토 목적 질의다.
...
sequenceDiagram
    ...
    U->>FE: "충북 음성 두성리 100에 공장 지을 수 있어?"
...
#### AI 기능모듈 상세 (활용·역할·입력·처리방식·출력)

| AI 기능모듈 | 활용 Tool/Function | 주요역할 | 입력 | 처리방식 | 출력 |
|---|---|---|---|---|---|
| **사전진단 판단 모듈** | 의미 이해 · 온톨로지 · Rule Engine · RAG · 무결성 검증 | 인허가 가능성 및 판정유형 판단 | 사용자 질의 · 필지정보 · 개발행위 · 토지특성정보 · 규제조건 | 온톨로지 3계층 지식구조 · Rule Engine 판정 · RAG 근거를 종합해 판단하며, 필요 시 지도제어·공간분석·3D 시뮬레이션 연계 결과를 보조 근거로 반영 | 가능 · 불가 · 보완 필요 · 조건부 등 검토 필요 |
| **RAG 근거 검색 모듈** | RAG 검색 · 온톨로지 · 외부 연계 | 판단에 필요한 법령·조례·지침·고시·업무처리기준·유사사례 검색 | 판단에 필요한 질의 키워드 · 행위정보 · 규제조건 | Vector 검색 · 키워드 검색 · Graph 검색 · 리랭킹을 수행하고, 필요 시 공간분석 조건을 검색 조건에 반영 | 근거 조문 · 관련 기준 · 유사사례 |
| **조건부 대안 생성 모듈** | 생성 · Rule Engine · RAG 검색 · 외부 연계 · 무결성 검증 | 불허·보완 조건에 대한 대안 제시 | 판정결과 · 불허·보완 필요사항 | 생성 기반 LLM · 대안 생성 템플릿 · 규제조건 재조합을 수행하며, 필요 시 공간분석·3D 시뮬레이션 연계 결과를 참조해 보완 조건과 대안 방향을 생성 | 보완 조건 · 대체 가능 조건 · 추가 확인 사항 |
| **민원세트 생성 모듈** | Rule Engine · 생성 | 신청 가능한 민원 목록 구성 | 행위유형 · 판정결과 · 관계기관 협의 필요 여부 | 민원 매핑 규칙과 절차 매핑을 적용하여 신청 대상 민원 · 선후행 절차 · 관계기관 협의 여부를 구성 | 민원 목록 · 선후행 절차 · 관계기관 |
| **민원 안내정보 생성 모듈** | 생성 · RAG 검색 · 외부 연계 | 민원인 대상 안내문 생성 | 민원 목록 · 제출서류 · 처리기준 | 공통 기반 LLM · 안내 템플릿 · 근거를 참조하여 신청 절차 · 제출서류 · 처리 기준 안내문을 구성 | 신청 절차 · 제출서류 · 처리 기준 안내 |
| **답변 무결성 검증 모듈** | 무결성 검증 · RAG 검색 · Rule Engine | 답변과 근거의 정합성 검증 | 생성 답변 · 판정결과 · 근거 조문 | 근거 일치성 검토 · 과잉·누락 검토 · 조건 충돌 여부를 확인 | 검증 통과 · 재검토 · 재생성 요청 |
| **사전진단 결과 보고서 생성 모듈** | 생성 · 무결성 검증 · 운영 거버넌스 | 최종 결과 보고서 작성 | 판정결과 · 근거 · 대안 · 민원안내 · 검증결과 | 공통 기반 LLM · 보고서 템플릿 · 검증 결과를 반영해 보고서를 생성하며, 공간분석·3D 시각화 자료가 있으면 부가자료로 포함(반영 시 정합성 추가 검토) | 사전진단 결과 보고서 |

> 위 표는 **목표(2차년도 포함) 기능모듈 명세**다. 현재 구현 대응: 사전진단 판단=`_verdict_judgment`+
> Rule(조건식), RAG 근거 검색=`ordinance_index`, 민원(세트·안내)=`permit_requirements`(요건·선후행),
> 나머지(조건부 대안 생성·답변 무결성 검증·보고서 생성)는 2차년도 계획(미구현).
...
| 말 | 뜻 |
|---|---|
| **`op`** | **동작 하나.** 지도에 시키는 일 한 가지 — "여기로 이동해라", "여기에 표시를 찍어라" |
| **`ops`** | **동작 목록.** `op` 를 여러 개 묶은 배열. `map_command` 가 받는 유일한 인자다 |

동작(`op`)은 7개다(map-mcp README §2).

| `op`(동작) | 뜻 | 인자 | 기본값 |
|---|---|---|---|
| `flyTo` | **그 위치로 이동** | `lat` · `lng` · `zoom` · `duration` | zoom 16 · duration 1.2 |
| `setZoom` | **확대 · 축소** | `level`(1~19) **또는** `delta`(±n) | delta 1 |
| `addMarker` | **표시를 찍음** | `lat` · `lng` · `label` · `focus` | focus false |
| `clearMarkers` | **표시를 모두 지움** | — | |
| `setBaseLayer` | **배경 지도 변경** | `style` — Base·white·midnight·Satellite·Hybrid | |
| `fitBounds` | **범위가 다 보이게 맞춤** | `south` · `west` · `north` · `east` | |
| `reset` | **처음 상태로 되돌림** | — | `clearMarkers` 를 함께 낸다 |
