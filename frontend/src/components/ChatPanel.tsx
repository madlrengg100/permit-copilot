...
const COLLAPSIBLE_SECTIONS: Array<{ test: RegExp; label: string }> = [
  ...
  { test: /분할\s*방법.*관련\s*조례.*법령\s*조문(?:\(근거\))?/, label: "분할 방법·근거 조문" },
  { test: /전국\s*공통\s*인허가\s*법령\s*조문(?:\(근거\))?/, label: "전국 공통 인허가 법령 조문" },
  { test: /(?:관련\s*)?(?:근거\s*법(?:령)?|법적\s*근거)|관련\s*(?:조례[·ㆍ\s]*)?법령\s*조문(?:\(근거\))?/, label: "관련 조례·법령 조문" },
  { test: /지자체\s*조례·지구단위계획\s*근거자료/, label: "지자체 조례·지구단위계획 근거자료" },
  { test: /지구단위계획\s*근거자료/, label: "지구단위계획 근거자료" },
  { test: /지자체\s*조례\s*근거자료/, label: "지자체 조례 근거자료" },
];
