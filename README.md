...
- **자연어로 기능·지도 제어** — 치수선·지적도·용도지역·경사도·팝업 켜기/끄기와 3D 모델 숨김/복원을
  말로 시킨다. 빠른 규칙이 즉시 처리하고, 규칙이 못 잡은 동의어·구어·오타 표현은 제미나이가 후속
  해석(`orchestrator._interpret_followup`의 `control` 필드)에 얹어 의미로 푼다(추가 LLM 호출 없음).
  숨김/표시는 마지막 제어 대상(last_control)을 기억해 "다시 켜/복원"이 항상 3D 모델이 아니라 마지막에
  끈 대상(치수선을 껐으면 치수선)을 되살린다.
- **표현 패턴 학습(세션 영속)** — 사용자가 새 표현을 쓰거나 "○○라고 하면 ○○하라는 뜻이야"처럼
  가르치면 제미나이가 이를 뽑아 제어 표현은 `control_glossary`, 이동 표현은 `nav_glossary`에 저장한다.
  세션 스냅샷에 포함돼 재시작·재접속에도 유지되고, 학습된 표현은 다음부터 빠른 경로에서 LLM 없이
  즉시 처리된다. 주소와 함께 온 찾기/이동 표현("○○동 ○○-○ 찾을 수 있어", "어디야")은 종합진단이
  아니라 그 필지로 이동한다(건축·모델 요청은 이동에서 제외).
- **지역 필지추천이 자연어 후속에서 도달** — "○○(시·군)에 농막·창고 지을 필지 리스트 뽑아줘"처럼
  지역+조건 탐색형 질의를 필지 진단 뒤 후속으로 물어도, 제미나이가 `_interpret_followup`의
  `recommend_region`(+`recommend_use`)으로 라우팅해 그 지역 비도시 용도지역을 공간스캔한 후보 필지
  리스트(지번·지목, 클릭 시 개별 진단)를 낸다 — '기능 없음'으로 회피하지 않는다. emit 단일원본은
  `orchestrator._recommend_areas_events()`(메인 도구루프·후속 공용)다. 후보 필지는 공간스캔이 만들며
  RAG(조례 벡터)는 근거 조문 검색용일 뿐 필지 DB가 아니다(역할 분리).
- **건축물대장·토지이용·토지소유 조회 복구(http→https)** — 공공데이터포털 `apis.data.go.kr` 이
  http로는 TCP 연결만 받고 HTTP 응답을 주지 않아(2026-08) 15초 타임아웃으로 건축물대장 조회가 통째로
