...
def apply(
    *,
    ...
    if front > 0 or adjacent > 0 or north_setback > 0:
        ...
        # 자료 미수집은 법적 0m가 아니다. 계산 단계에서는 후퇴를 적용하지 않더라도 결과 문구는
        # 반드시 '미확정'으로 유지해 화면용 기본값이 법적 판정으로 승격되지 않게 한다.
        setback_caveat = (
            f"{jurisdiction or '이 지역'} 건축조례의 대지 안의 공지 값은 별표(첨부)라 "
            f"자동수집되지 않아 {building_use}의 법적 이격거리는 미확정입니다. 현재 배치 계산에는 "
            "후퇴를 적용하지 않았으나 이를 0m 기준으로 해석하면 안 되며, 조례 별표를 확인해야 합니다. "
        )
    else:
