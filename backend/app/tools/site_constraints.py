...
def apply(
    *,
    ...
    if front > 0 or adjacent > 0 or north_setback > 0:
        ...
        # 대지 안의 공지 값은 조례 별표(첨부)에 있어 자동수집이 어렵다. 다만 건축법 시행령
        # 별표2는 단독주택·소규모 근린생활시설 등 '조례 위임 용도'에는 이격을 강제하지 않으므로
        # (영 별표2의 범위 안에서 조례가 정하고, 위임 용도엔 미적용) 이 경우 0m가 통상 맞다.
        # 별표2가 직접 규정하는 아파트·대형 판매·숙박·위락시설 등만 조례 별표 값 확인이 필요하다.
        setback_caveat = (
            f"{jurisdiction or '이 지역'} 건축조례의 대지 안의 공지 값은 별표(첨부)라 "
            f"자동수집되지 않았습니다. {building_use} 등 단독주택·소규모 근린생활시설은 건축법 "
            "시행령 별표2상 이격이 조례에 위임(강제 규정 없음)돼 통상 이격이 없거나 작아 0m로 "
            "계산했으며, 아파트·대형 판매·숙박·위락시설 등은 조례 별표로 이격이 정해지니 "
            "그 별표를 개별 확인해야 합니다. "
        )
    else:
